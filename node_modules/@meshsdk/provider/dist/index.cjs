...
var U5CProvider = class {
   ...
   */
  async submitTx(tx) {
    const cbor = (0, import_common8.toBytes)(tx);
  ...
  async fetchUTxOs(hash, index) {
    const utxoSearchResult = await this.queryClient.readUtxosByOutputRef(
      [{
        txHash: (0, import_common8.hexToBytes)(hash),
        outputIndex: index || 0
        // TODO: handle case when index is not provided. Note: readUtxos might not support this, try when readTx is implemented
      }]
    );
    return utxoSearchResult.map((item) => {
